...
class BodyTypeFormsTest extends TestCase
{
    ...
    public function test_cargo_owner_picks_body_types_for_the_vehicle_class_and_load_kind(): void
    {
        ...
        $c = Volt::test('cargo-owner.loads.create')->set('currentStep', 2)
            ...
            ->set('body_types', ['tenteli', 'uzun_dorse'])
            ->set('vehicle_type', 'kamyonet')->assertDontSee('Uzun dorse (13.60)');
        $this->assertSame([], $c->get('body_types'), 'Araç sınıfı değişince tıra özel seçimler düşer');
    ...
    public function test_driver_saves_body_and_trailer_length_on_the_vehicle(): void
    {
        ...
        // Kamyonete dorse uzunluğu yazılmaz; liftli kamyonet olur, damperli olmaz (sınıf dışı → hata)
        Volt::test('driver.vehicles.index')->call('openEdit', $v->id)->set('vehicle_type', 'kamyonet')->assertDontSee('Dorse uzunluğu')
            ->set('body_type', 'liftli')->set('trailer_length', 'uzun')->call('save')->assertHasNoErrors();
        $v->refresh();
        $this->assertSame(['kamyonet', 'liftli', null], [$v->vehicle_type, $v->body_type, $v->trailer_length]);
