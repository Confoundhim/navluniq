...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('cargo_owner_profiles', function (Blueprint $table) {
            ...
            $table->string('tax_no', 10)->nullable()->unique();
            $table->string('tc_no', 11)->nullable()->unique();
            $table->boolean('nvi_verified')->default(false);
            $table->boolean('gib_verified')->default(false);
            $table->string('kyc_status', 24)->default('unsubmitted')->index();
        ...
        Schema::create('driver_profiles', function (Blueprint $table) {
            ...
            foreach (['avatar_path', 'driver_license_path', 'src_document_path', 'psychotechnic_path', 'k_document_path', 'liability_insurance_path', 'selfie_with_id_path'] as $column) {
        ...
        Schema::create('kyc_documents', function (Blueprint $table) {
            ...
            $table->string('storage_disk', 64)->default('kyc_private');
        ...
        Schema::create('driver_vehicles', function (Blueprint $table) {
            ...
            $table->string('brand');
            $table->string('model');
            $table->string('vehicle_type', 48)->index();
            $table->string('ruhsat_path')->nullable();
            $table->string('vehicle_photo_path')->nullable();
            $table->boolean('is_active')->default(true)->index();
            $table->timestamps();
            $table->softDeletes();
            $table->index(['driver_profile_id', 'is_active']);
        });
        Schema::create('loads', function (Blueprint $table) {
            $table->id();
            $table->uuid('public_id')->nullable()->unique();
            $table->foreignId('cargo_owner_profile_id')->constrained()->restrictOnDelete();
            $table->foreignId('driver_profile_id')->nullable()->constrained()->nullOnDelete();
            $table->string('source_type', 24)->default('internal')->index();
            $table->string('visibility', 24)->default('private')->index();
            $table->string('pickup_location');
            $table->string('delivery_location');
            $table->geometry('pickup_coordinates', subtype: 'point', srid: 4326)->nullable();
            $table->geometry('delivery_coordinates', subtype: 'point', srid: 4326)->nullable();
            ...
            $table->string('escrow_status', 32)->default('pending_payment')->index();
            $table->string('status', 32)->default('active_seeking')->index();
            $table->text('dispute_reason')->nullable();
            $table->text('rejection_reason')->nullable();
            $table->string('e_irsaliye_no')->nullable()->index();
            $table->timestamp('published_at')->nullable()->index();
            $table->timestamp('cancelled_at')->nullable();
            $table->timestamps();
            $table->softDeletes();
            $table->index(['cargo_owner_profile_id', 'status', 'pickup_date']);
        ...
        Schema::create('offers', function (Blueprint $table) {
            ...
            $table->timestamps();
            $table->softDeletes();
            $table->index(['load_id', 'status', 'created_at']);
        });
        Schema::create('shipments', function (Blueprint $table) {
            ...
            $table->foreignId('driver_profile_id')->constrained()->restrictOnDelete();
            $table->foreignId('vehicle_id')->nullable()->constrained('driver_vehicles')->nullOnDelete();
            $table->string('status', 32)->default('awaiting_pickup')->index();
            ...
            $table->timestamp('auto_approval_due_at')->nullable()->index();
            $table->timestamps();
        });
        Schema::create('driver_locations', function (Blueprint $table) {
            ...
            $table->decimal('speed', 7, 2)->default(0);
            $table->decimal('heading', 6, 2)->default(0);
    ...
    {
        foreach (['messages', 'conversation_participants', 'conversations', 'shipment_evidence', 'driver_locations', 'shipments', 'offers', 'loads', 'driver_vehicles', 'user_consents', 'kyc_documents', 'driver_profiles', 'cargo_owner_profiles'] as $table) {
