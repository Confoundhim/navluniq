...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('scrapers', function (Blueprint $table) {
            ...
            $table->timestamps();
            $table->softDeletes();
            $table->unique(['type', 'source_identifier']);
        });
        Schema::create('source_permissions', function (Blueprint $table) {
            $table->id();
            $table->foreignId('scraper_id')->unique()->constrained()->cascadeOnDelete();
            $table->string('permission_basis', 64);
        ...
        Schema::create('scraped_loads', function (Blueprint $table) {
            ...
            $table->foreignId('source_permission_id')->nullable()->constrained()->nullOnDelete();
            $table->char('content_hash', 64)->nullable()->unique();
            $table->text('raw_message');
            $table->string('sender_phone')->nullable();
            $table->text('encrypted_sender_phone')->nullable();
            ...
            $table->string('status', 32)->default('raw_unprocessed')->index();
            $table->string('parsed_by_llm')->nullable();
            $table->decimal('parse_confidence', 5, 4)->nullable();
            $table->string('visibility', 24)->default('private')->index();
            $table->timestamp('available_to_free_at')->nullable()->index();
            $table->timestamp('retention_expires_at')->nullable()->index();
            $table->json('parse_metadata')->nullable();
            $table->timestamps();
            $table->softDeletes();
            $table->index(['scraper_id', 'status', 'created_at']);
        });
        Schema::create('ai_provider_usage', function (Blueprint $table) {
            ...
            $table->boolean('quota_exhausted')->default(false)->index();
            $table->timestamp('quota_resets_at')->nullable();
            $table->decimal('estimated_cost', 19, 6)->default(0);
        ...
        Schema::create('ai_parse_attempts', function (Blueprint $table) {
            $table->id();
            $table->foreignId('scraped_load_id')->constrained()->cascadeOnDelete();
    ...
    {
        foreach (['ai_parse_attempts', 'ai_provider_usage', 'scraped_loads', 'source_permissions', 'scrapers'] as $table) {
