<div class="max-w-4xl mx-auto space-y-5" wire:poll.15s>
    ...
    <div class="space-y-2">
        @forelse($notifications as $n)
            <div class="rounded-2xl border p-4 bg-white dark:bg-neutral-900 {{ $n->isRead() ? 'border-neutral-200 dark:border-neutral-800' : 'border-brand-500/30 bg-brand-500/[0.03]' }}">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0 flex-1 space-y-1">
                        ...
                        <div class="flex flex-wrap items-center gap-3 pt-1">
                            ...
                            @endif
                            @if(! $n->isRead())
                                <button type="button" wire:click="markRead({{ $n->id }})" class="text-[11px] text-neutral-400 hover:text-neutral-600">Okundu işaretle</button>
                            @endif
                        </div>
                    </div>
                </div>
