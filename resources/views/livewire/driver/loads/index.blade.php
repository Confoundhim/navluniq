...
<div class="space-y-6">
    ...
    <div class="border-b border-neutral-800 pb-4 flex flex-col sm:flex-row sm:items-end justify-between gap-3">
        <div>
            <h2 class="text-xl font-bold text-white tracking-tight">İlan Havuzu ve Tekliflerim</h2>
            <p class="text-xs text-neutral-400 mt-1">Açık ilanlara teklif verin, tekliflerinizi takip edin ve dış kaynaklı ilanları inceleyin.</p>
        </div>
        <div class="flex flex-wrap gap-2 text-xs">
            @foreach(['pool' => 'İlan havuzu', 'offers' => 'Tekliflerim', 'external' => 'Dış kaynak ilanlar'] as $key => $label)
                <button type="button" wire:click="setTab('{{ $key }}')"
                    class="px-4 py-2 rounded-xl font-bold border transition-colors {{ $tab === $key ? 'bg-brand-500/10 border-brand-500/30 text-brand-400' : 'bg-neutral-900 border-neutral-800 text-neutral-400 hover:text-white' }}">
                    {{ $label }}
                </button>
            @endforeach
        </div>
    ...
    @if($tab === 'pool')
        <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                <div>
                    <label class="block font-medium text-neutral-300 mb-1">Rota ara</label>
                    <input type="text" wire:model.live.debounce.400ms="search" placeholder="Şehir veya ilçe" class="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-2.5 text-white focus:border-brand-500 focus:outline-none">
                </div>
                <div>
                    <label class="block font-medium text-neutral-300 mb-1">Araç türü</label>
                    <select wire:model.live="vehicleType" class="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2.5 text-white focus:border-brand-500 focus:outline-none">
            ...
            <div class="space-y-3">
                @forelse($loads as $load)
                    <div class="p-4 bg-neutral-950 border border-neutral-800 hover:border-brand-500/40 rounded-xl transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs">
                        ...
                        <div class="flex sm:flex-col items-center sm:items-end justify-between gap-2 shrink-0 border-t sm:border-t-0 border-neutral-800 pt-3 sm:pt-0">
                            <div class="text-lg font-black text-white font-mono">{{ number_format((float) ($load->price ?? 0), 2, ',', '.') }} ₺</div>
                ...
                @empty
                    <div class="p-6 bg-neutral-950 border border-dashed border-neutral-800 rounded-xl text-center text-xs text-neutral-400">Filtrelerinize uyan açık ilan bulunmuyor.</div>
                @endforelse
            </div>
    ...
    @if($tab === 'offers')
        <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 space-y-3">
            @forelse($offers as $offer)
                ...
                <div class="p-4 bg-neutral-950 border border-neutral-800 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs">
                    <div class="space-y-1.5 flex-1">
                        ...
                        <div class="text-neutral-400">
                            Teklifim: <span class="text-white font-mono font-bold">{{ number_format((float) ($offer->amount ?? 0), 2, ',', '.') }} ₺</span>
                    ...
                    <div class="flex sm:flex-col items-center sm:items-end justify-between gap-2 shrink-0 border-t sm:border-t-0 border-neutral-800 pt-3 sm:pt-0">
                        <span class="px-2.5 py-1 rounded-full text-[10px] font-bold border
                            {{ $offer->status === 'accepted' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : ($offer->status === 'pending' ? 'bg-amber-500/10 border-amber-500/20 text-amber-400' : 'bg-neutral-800 border-neutral-700 text-neutral-300') }}">
            ...
            @empty
                <div class="p-6 bg-neutral-950 border border-dashed border-neutral-800 rounded-xl text-center text-xs text-neutral-400">Henüz teklif vermediniz.</div>
            @endforelse
    ...
    @if($tab === 'external')
        <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 space-y-4">
            ...
            <div class="space-y-3">
                ...
                @empty
                    <div class="p-6 bg-neutral-950 border border-dashed border-neutral-800 rounded-xl text-center text-xs text-neutral-400">Şu anda görüntülenebilir dış kaynak ilan yok.</div>
                @endforelse
            </div>
    ...
    @if($offerModalOpen && $selectedLoad)
        <div class="fixed inset-0 z-[9999] overflow-y-auto flex items-start sm:items-center justify-center p-4">
            <div class="fixed inset-0 bg-neutral-950/85 backdrop-blur-md" wire:click="closeOffer"></div>
            <form wire:submit.prevent="submitOffer" class="relative z-10 w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-2xl space-y-4 text-left text-xs">
                <div class="border-b border-neutral-800 pb-3">
                    <h3 class="text-base font-bold text-white">Teklif ver</h3>
                    <p class="text-neutral-400 mt-0.5">{{ $selectedLoad->pickup_location }} &rarr; {{ $selectedLoad->delivery_location }} · İlan fiyatı {{ number_format((float) ($selectedLoad->price ?? 0), 2, ',', '.') }} ₺</p>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        <label class="block font-medium text-neutral-300 mb-1">Teklif tutarı (₺)</label>
                        <input type="number" step="0.01" min="{{ $minPrice }}" wire:model="amount" class="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-2.5 text-white font-mono focus:border-brand-500 focus:outline-none">
                        @error('amount') <span class="text-rose-500 text-[11px] mt-1 block">{{ $message }}</span> @enderror
                        <span class="text-[10px] text-neutral-500 mt-1 block">Asgari {{ number_format($minPrice, 2, ',', '.') }} ₺</span>
                    </div>
                    <div>
                        <label class="block font-medium text-neutral-300 mb-1">Tahmini süre (gün)</label>
                        <input type="number" min="1" max="30" wire:model="estimated_days" class="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-2.5 text-white font-mono focus:border-brand-500 focus:outline-none">
                        @error('estimated_days') <span class="text-rose-500 text-[11px] mt-1 block">{{ $message }}</span> @enderror
                    </div>
                </div>

                <div>
                    <label class="block font-medium text-neutral-300 mb-1">Mesaj (isteğe bağlı)</label>
                    <textarea wire:model="message" rows="3" maxlength="1000" class="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-2.5 text-white focus:border-brand-500 focus:outline-none"></textarea>
                ...
                @if(! $kycApproved)
                    ...
                    <div class="flex gap-3 pt-2">
                        <button type="button" wire:click="closeOffer" class="flex-1 px-4 py-2.5 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-neutral-300 font-semibold">Kapat</button>
                    </div>
                @else
                    @if(! $hasActiveVehicle)
                        <div class="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300">
                    ...
                    @endif
                    <div class="flex gap-3 pt-2">
                        <button type="button" wire:click="closeOffer" class="flex-1 px-4 py-2.5 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-neutral-300 font-semibold">Vazgeç</button>
