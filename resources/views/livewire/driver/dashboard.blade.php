...
<div class="space-y-6">
    ...
    <div class="border-b border-neutral-800 pb-4">
        <h2 class="text-xl font-bold text-white tracking-tight">Hoş geldiniz, {{ auth()->user()->first_name }}</h2>
        <p class="text-xs text-neutral-400 mt-1">Tekliflerinizin, aktif sevkiyatınızın ve hakedişlerinizin özeti.</p>
    </div>
    ...
    @if($kycStatus !== 'approved')
        <div class="p-4 rounded-xl border text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3
            ...
            <a href="{{ route('driver.profile.index') }}" wire:navigate class="shrink-0 px-4 py-2 rounded-xl bg-neutral-900 border border-neutral-700 text-white font-bold hover:bg-neutral-800">Belgelere git</a>
        </div>
    @endif

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
            <div class="text-xs text-neutral-400">Değerlendirilen tekliflerim</div>
            <div class="mt-2 text-2xl font-black text-white font-mono">{{ (int) $pendingOffers }}</div>
        ...
        <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
            ...
            <div class="mt-2 text-2xl font-black text-white font-mono">{{ number_format((float) ($wallet['in_escrow'] ?? 0), 2, ',', '.') }} ₺</div>
            <div class="mt-2 text-[11px] text-neutral-500">Yük sahibinin havuza yatırdığı navlun bedeli</div>
        </div>
        <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
            <div class="text-xs text-neutral-400">Ödeme sırasındaki hakediş</div>
            <div class="mt-2 text-2xl font-black text-white font-mono">{{ number_format((float) ($wallet['pending'] ?? 0), 2, ',', '.') }} ₺</div>
        ...
        <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
            <div class="text-xs text-neutral-400">Ödenen toplam hakediş</div>
            <div class="mt-2 text-2xl font-black text-emerald-400 font-mono">{{ number_format((float) ($wallet['paid'] ?? 0), 2, ',', '.') }} ₺</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 space-y-4">
                <h3 class="text-xs font-bold text-neutral-400 uppercase tracking-wider">Aktif sevkiyat</h3>
                ...
                @if($activeLoad)
                    <div class="p-4 bg-neutral-950 border border-neutral-800 rounded-xl space-y-3 text-xs">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                            ...
                            <span class="px-2.5 py-1 rounded-full bg-brand-500/10 border border-brand-500/20 text-brand-400 font-bold text-[10px]">{{ $activeLoad->statusLabel() }}</span>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-neutral-400">
                            <div>Yük sahibi: <span class="text-white font-semibold">{{ $activeLoad->cargoOwnerProfile?->displayName() ?: 'Belirtilmemiş' }}</span></div>
                            <div>Yükleme: <span class="text-white">{{ $activeLoad->pickup_date?->format('d.m.Y H:i') ?? 'Belirtilmemiş' }}</span></div>
                            <div>Navlun: <span class="text-white font-mono font-bold">{{ number_format((float) ($activeLoad->price ?? 0), 2, ',', '.') }} ₺</span></div>
                            <div>Ödeme: <span class="text-white">{{ $activeLoad->escrowLabel() }}</span></div>
                        </div>
                        <div class="pt-2 border-t border-neutral-800 flex flex-col sm:flex-row gap-2">
            ...
            <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 space-y-4">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                        <h3 class="text-xs font-bold text-neutral-400 uppercase tracking-wider">Son ilanlar</h3>
                ...
                @forelse($recentLoads as $load)
                    <div class="p-4 bg-neutral-950 border border-neutral-800 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                        <div class="space-y-1">
                        ...
                        <div class="text-white font-mono font-bold text-base shrink-0">{{ number_format((float) ($load->price ?? 0), 2, ',', '.') }} ₺</div>
                    </div>
                @empty
                    <div class="p-6 bg-neutral-950 border border-dashed border-neutral-800 rounded-xl text-center text-xs text-neutral-400">Henüz açık ilan yok.</div>
                @endforelse
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 space-y-3 text-xs">
                <h3 class="text-xs font-bold text-neutral-400 uppercase tracking-wider">Belge durumu</h3>
                <div class="flex items-center justify-between">
                    <span class="text-neutral-400">KYC</span>
            ...
            <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 space-y-3 text-xs">
                <h3 class="text-xs font-bold text-neutral-400 uppercase tracking-wider">Premium</h3>
                @if($profile?->isPremium())
                    <div class="text-emerald-400 font-bold">Aktif</div>
                    <div class="text-neutral-400">{{ $profile->premium_until->format('d.m.Y H:i') }} tarihine kadar geçerli.</div>
                @else
                    <div class="text-neutral-300 font-bold">Pasif</div>
                    <div class="text-neutral-400">Komisyon oranınız: %{{ number_format($profile?->commissionRate() ?? 0, 1, ',', '.') }}</div>
                @endif
            ...
            <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 space-y-3 text-xs">
                <h3 class="text-xs font-bold text-neutral-400 uppercase tracking-wider">Aktif araç</h3>
                @if($profile?->activeVehicle)
                    <div class="text-white font-mono font-bold">{{ $profile->activeVehicle->plate }}</div>
                    <div class="text-neutral-400">{{ $profile->activeVehicle->brand }} {{ $profile->activeVehicle->model }}</div>
                @else
                    <div class="text-neutral-400">Aktif aracınız yok. Teklif verebilmek için bir araç ekleyip aktif yapın.</div>
                @endif
