...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-neutral-800 pb-4">
        <div>
            <a href="{{ route('cargo-owner.loads.index') }}" wire:navigate class="text-xs text-neutral-400 hover:text-brand-400 font-semibold inline-flex items-center gap-1.5 mb-1 transition-colors">
                &larr; İlanlarıma dön
            </a>
    ...
    @if($load)
        <div class="p-5 rounded-2xl bg-neutral-900 border border-neutral-800 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
            <div class="lg:col-span-2 min-w-0">
                <span class="text-neutral-500 block">Rota</span>
                <span class="text-white font-bold break-words">{{ $load->pickup_location }} &rarr; {{ $load->delivery_location }}</span>
            </div>
            <div>
                <span class="text-neutral-500 block">Araç & yük</span>
                <span class="text-neutral-200 font-medium">{{ $vehicleTypes[$load->vehicle_type] ?? $load->vehicle_type }} · {{ $load->goods_type }}</span>
            ...
            <div>
                ...
                <span class="text-neutral-500 block mt-1">Durum: <span class="text-neutral-300">{{ $load->statusLabel() }}</span></span>
            </div>
        </div>

        @if($load->status !== 'active_seeking')
            <div class="p-4 rounded-xl bg-neutral-900 border border-neutral-800 text-xs text-neutral-300 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <span>Bu ilan artık teklif kabul etmiyor. Durum: <strong class="text-white">{{ $load->statusLabel() }}</strong></span>
                ...
                @elseif(in_array($load->status, ['driver_assigned', 'on_the_way', 'delivered', 'disputed', 'completed'], true))
                    <a href="{{ route('cargo-owner.shipments.show', $load->id) }}" wire:navigate class="px-4 py-2 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-white font-semibold text-center">Sevkiyatı görüntüle</a>
                @endif
            </div>
        @endif
        ...
        <div class="space-y-4">
                ...
                <div class="bg-neutral-900 border border-neutral-800 hover:border-neutral-700/80 rounded-2xl p-6 transition-all duration-200 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    ...
                    <div class="space-y-3 flex-1 min-w-0">
                            ...
                            <div class="min-w-0">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    @if($driver?->isKycApproved())
                                        <span class="px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-bold">Belgeleri doğrulandı</span>
                                    @endif
                                </div>
                        ...
                        <div class="flex flex-wrap items-center gap-4 text-xs text-neutral-400 pt-1">
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <span class="text-neutral-200 font-medium">{{ $vehicle ? (trim(($vehicle->brand ?? '').' '.($vehicle->model ?? '')) ?: '—').' · '.($vehicleTypes[$vehicle->vehicle_type] ?? $vehicle->vehicle_type) : 'Aktif araç bilgisi yok' }}</span>
                            </div>
                            <div class="flex items-center gap-1">
                                <span class="text-neutral-500">Tahmini süre:</span>
                                <span class="text-neutral-200 font-medium">{{ $offer->estimated_days ? $offer->estimated_days.' gün' : 'Belirtilmedi' }}</span>
                            </div>
                            <div class="flex items-center gap-1">
                                <span class="text-neutral-500">Teklif tarihi:</span>
                                <span class="text-neutral-200">{{ $offer->created_at?->format('d.m.Y H:i') }}</span>
                            </div>
                            @if($offer->expires_at)
                                <div class="flex items-center gap-1">
                                    <span class="text-neutral-500">Geçerlilik:</span>
                                    <span class="text-neutral-200">{{ $offer->expires_at->format('d.m.Y H:i') }}</span>
                                </div>
                            @endif
                        </div>

                        @if($offer->message)
                            <p class="text-xs text-neutral-300 bg-neutral-950/60 p-3 rounded-xl border border-neutral-800/80 leading-relaxed break-words">{{ $offer->message }}</p>
                        @endif
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row lg:flex-col items-start sm:items-center lg:items-end justify-between gap-4 border-t lg:border-t-0 pt-4 lg:pt-0 border-neutral-800">
                        <div class="text-left lg:text-right">
                            <span class="text-[10px] text-neutral-500 uppercase tracking-wider block">Teklif tutarı</span>
                        ...
                        @if($load->status === 'active_seeking')
                            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                                <button type="button" wire:click="rejectOffer({{ $offer->id }})" wire:confirm="Bu teklifi reddetmek istediğinize emin misiniz?" wire:loading.attr="disabled" class="px-4 py-2.5 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-neutral-300 text-xs font-semibold transition-colors">
                                    Reddet
                                </button>
            ...
            @empty
                <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-12 text-center space-y-3">
                    <h4 class="text-base font-bold text-white">{{ $load->status === 'active_seeking' ? 'Henüz bekleyen teklif yok' : 'Bekleyen teklif yok' }}</h4>
        ...
        @if($closedOffers->isNotEmpty())
            <div x-data="{ open: false }" class="bg-neutral-900 border border-neutral-800 rounded-2xl">
                <button type="button" x-on:click="open = !open" class="w-full flex items-center justify-between p-4 text-xs font-semibold text-neutral-300 hover:text-white">
                    <span>Kapanmış teklifler ({{ $closedOffers->count() }})</span>
                    <span x-text="open ? 'Gizle' : 'Göster'" class="text-neutral-500"></span>
                </button>
                <div x-show="open" x-cloak class="border-t border-neutral-800 divide-y divide-neutral-800">
                    @foreach($closedOffers as $offer)
                        <div class="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                            <div class="min-w-0">
                                <span class="text-neutral-200 font-semibold">{{ $offer->driverProfile?->user?->full_name ?: 'Şoför' }}</span>
                                <span class="text-neutral-500"> · {{ $offer->created_at?->format('d.m.Y H:i') }}</span>
                            </div>
                            <div class="flex items-center gap-3">
                                <span class="font-mono text-neutral-300">{{ number_format((float) $offer->amount, 2, ',', '.') }} ₺</span>
                                <span class="px-2 py-0.5 rounded-full bg-neutral-800 border border-neutral-700 text-neutral-400 text-[10px] font-bold">{{ \App\Models\Offer::STATUS_LABELS[$offer->status] ?? $offer->status }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
