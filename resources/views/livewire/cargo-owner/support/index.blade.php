...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-neutral-800 pb-4">
        <div>
            <h2 class="text-xl font-bold text-white tracking-tight">Destek taleplerim</h2>
            <p class="text-xs text-neutral-400 mt-1">Teknik, finansal veya operasyonel sorularınız için destek bileti açın; yanıtlar bu sayfada görünür.</p>
        </div>
    ...
    <div class="space-y-4">
            ...
            <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 space-y-4">
                ...
                <div class="flex flex-wrap items-center justify-between gap-4 border-b border-neutral-800 pb-3">
                    <div class="flex flex-wrap items-center gap-3">
                        <span class="px-2.5 py-1 rounded-md bg-neutral-800 text-neutral-300 font-mono text-xs font-bold">#{{ $ticket->id }}</span>
                        <span class="px-2.5 py-0.5 rounded-full bg-brand-500/10 text-brand-400 border border-brand-500/20 text-[10px] font-bold">{{ $categories[$ticket->category] ?? $ticket->subject ?? $ticket->category }}</span>
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold border
                            {{ $ticket->status === 'open' ? 'bg-amber-500/10 border-amber-500/20 text-amber-400' : ($ticket->status === 'answered' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-neutral-800 border-neutral-700 text-neutral-400') }}">
                ...
                <div class="text-xs text-neutral-300 leading-relaxed bg-neutral-950 p-4 rounded-xl border border-neutral-800 break-words">
                ...
                @if($ticket->admin_reply)
                    <div class="text-xs text-neutral-200 leading-relaxed bg-brand-500/5 p-4 rounded-xl border border-brand-500/20 space-y-1">
                        ...
                        <p class="text-neutral-300 pt-1 break-words">{{ $ticket->admin_reply }}</p>
                    </div>
        ...
        @empty
            <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-12 text-center space-y-4">
                ...
                <div class="space-y-1">
                    <h4 class="text-base font-bold text-white">Henüz destek talebiniz yok</h4>
                    <p class="text-xs text-neutral-400 max-w-sm mx-auto">İşlemlerinizle ilgili her konuda bilet oluşturabilirsiniz.</p>
                </div>
            </div>
        @endforelse
    </div>
    ...
    @if($newTicketModalOpen)
        <div class="fixed inset-0 z-[9999] overflow-y-auto flex items-start sm:items-center justify-center p-4">
            <div class="fixed inset-0 bg-neutral-950/85 backdrop-blur-md" wire:click="$set('newTicketModalOpen', false)"></div>
            <form wire:submit.prevent="createTicket" class="relative z-10 w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-2xl space-y-5 text-left">

                <div class="flex items-center justify-between border-b border-neutral-800 pb-4">
                    <h3 class="text-base font-bold text-white">Yeni destek bileti</h3>
                    <button type="button" wire:click="$set('newTicketModalOpen', false)" class="text-neutral-400 hover:text-white">
                ...
                <div class="space-y-4 text-xs">
                    <div>
                        <label class="block font-medium text-neutral-300 mb-1.5">Kategori <span class="text-brand-500">*</span></label>
                    ...
                    <div>
                        <label class="block font-medium text-neutral-300 mb-1.5">Mesajınız <span class="text-brand-500">*</span></label>
                        <textarea wire:model="message" rows="5" maxlength="3000" placeholder="Yaşadığınız durumu veya sorunuzu ayrıntılı açıklayın." class="w-full bg-neutral-950 border border-neutral-800 rounded-xl p-3 text-white placeholder-neutral-600 focus:border-brand-500 focus:outline-none"></textarea>
                ...
                <div class="flex flex-col sm:flex-row gap-3 pt-1">
                    <button type="button" wire:click="$set('newTicketModalOpen', false)" class="flex-1 px-4 py-2.5 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-neutral-300 text-xs font-semibold transition-colors">Kapat</button>
