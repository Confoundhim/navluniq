...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-neutral-800 pb-4">
        <div>
            ...
            <p class="text-xs text-neutral-400 mt-1">Hasar, eksik teslimat veya gecikme durumunda uyuşmazlık açın; havuzdaki ödeme karar verilene kadar askıya alınır.</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="p-4 rounded-xl bg-neutral-900 border border-neutral-800 space-y-1">
            ...
            <p class="text-[11px] text-neutral-400 leading-relaxed">Uyuşmazlık açıldığında havuzdaki navlun bedeli karar verilene kadar şoföre aktarılmaz.</p>
        </div>
        <div class="p-4 rounded-xl bg-neutral-900 border border-neutral-800 space-y-1">
            <div class="text-xs font-bold text-brand-400">2. Şoför savunma yapar</div>
            <p class="text-[11px] text-neutral-400 leading-relaxed">Şoför, açıklamanızı görüp savunmasını ve kendi kanıtlarını yükler.</p>
        </div>
        <div class="p-4 rounded-xl bg-neutral-900 border border-neutral-800 space-y-1">
            <div class="text-xs font-bold text-brand-400">3. Karar verilir</div>
            <p class="text-[11px] text-neutral-400 leading-relaxed">NavlunIQ ekibi kanıtları inceler; karara göre bedel şoföre ödenir veya size iade edilir.</p>
        </div>
    </div>
            ...
            <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 space-y-4">
                ...
                <div class="flex flex-wrap items-center justify-between gap-4 border-b border-neutral-800 pb-3">
                    <div class="flex flex-wrap items-center gap-3">
                        <span class="px-2.5 py-1 rounded-md bg-neutral-800 text-neutral-300 font-mono text-xs font-bold">Uyuşmazlık #{{ $dispute->id }}</span>
                        <span class="px-2.5 py-0.5 rounded-full border text-[10px] font-bold
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 text-xs">
                    <div class="space-y-2">
                        ...
                        <div class="p-3 bg-neutral-950 rounded-xl border border-neutral-800 space-y-1">
                            @if($load)
                                <div class="text-white font-bold break-words">#{{ $load->id }} · {{ $load->pickup_location }} &rarr; {{ $load->delivery_location }}</div>
                                <div class="text-neutral-500">Şoför: <span class="text-neutral-300 font-medium">{{ $load->driverProfile?->user?->full_name ?: '—' }}</span></div>
                                <div class="text-neutral-500">Havuzdaki tutar: <span class="text-brand-400 font-mono font-bold">{{ number_format((float) ($load->price ?? 0), 2, ',', '.') }} ₺</span> · {{ $load->escrowLabel() }}</div>
                    ...
                    <div class="space-y-2">
                        <div class="text-neutral-400">Açıklamanız</div>
                        <div class="p-3 bg-neutral-950 rounded-xl border border-neutral-800 text-neutral-300 leading-relaxed break-words">{{ $dispute->cargo_owner_claim }}</div>
                ...
                @if($dispute->driver_defense || $dispute->driver_proof_photo_path)
                    <div class="p-4 rounded-xl bg-neutral-950 border border-neutral-800 space-y-2 text-xs">
                        <span class="font-bold text-neutral-300">Şoförün savunması</span>
                        @if($dispute->driver_defense)
                            <p class="text-neutral-300 leading-relaxed break-words">{{ $dispute->driver_defense }}</p>
                        @endif
                ...
                @if($dispute->status !== 'open')
                    <div class="p-4 rounded-xl bg-neutral-950/80 border border-brand-500/20 space-y-1 text-xs">
                        ...
                        <p class="text-neutral-300 leading-relaxed break-words">{{ $dispute->arbitration_notes ?: 'Karar notu girilmedi.' }}</p>
                    </div>
                @endif

            </div>
        @empty
            <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-12 text-center space-y-3">
                <h4 class="text-base font-bold text-white">Henüz uyuşmazlık kaydınız yok</h4>
                <p class="text-xs text-neutral-400 max-w-sm mx-auto">Yolda veya teslim edilmiş ve ödemesi havuzda bekleyen sevkiyatlarınız için uyuşmazlık açabilirsiniz.</p>
            </div>
        @endforelse
    </div>
    ...
    @if($createModalOpen)
        <div class="fixed inset-0 z-[9999] overflow-y-auto flex items-start sm:items-center justify-center p-4">
            <div class="fixed inset-0 bg-neutral-950/85 backdrop-blur-md" wire:click="$set('createModalOpen', false)"></div>
            <form wire:submit.prevent="submitDispute" class="relative z-10 w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-2xl space-y-5 text-left">

                <div class="flex items-center justify-between border-b border-neutral-800 pb-4">
                    <h3 class="text-base font-bold text-white">Uyuşmazlık aç</h3>
                    <button type="button" wire:click="$set('createModalOpen', false)" class="text-neutral-400 hover:text-white">
                ...
                <div class="space-y-4 text-xs">
                    <div>
                        <label class="block font-medium text-neutral-300 mb-1.5">Sevkiyat <span class="text-brand-500">*</span></label>
                    ...
                    <div>
                        <label class="block font-medium text-neutral-300 mb-1.5">Yaşanan sorun <span class="text-brand-500">*</span></label>
                        <textarea wire:model="claim" rows="4" maxlength="3000" placeholder="Hasar, eksik miktar veya gecikme gibi durumu ayrıntılı yazın." class="w-full bg-neutral-950 border border-neutral-800 rounded-xl p-3 text-white placeholder-neutral-600 focus:border-brand-500 focus:outline-none"></textarea>
                        @error('claim') <span class="text-rose-500 text-[11px] mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block font-medium text-neutral-300 mb-1.5">Fotoğraf / belge (isteğe bağlı, JPG, PNG, PDF)</label>
                        <input type="file" wire:model="claim_photo" accept="image/jpeg,image/png,application/pdf" class="w-full text-xs text-neutral-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-neutral-800 file:text-neutral-200 hover:file:bg-neutral-700 cursor-pointer">
                        <div wire:loading wire:target="claim_photo" class="text-[10px] text-neutral-500 mt-1">Dosya hazırlanıyor...</div>
                ...
                <div class="flex flex-col sm:flex-row gap-3 pt-1">
                    <button type="button" wire:click="$set('createModalOpen', false)" class="flex-1 px-4 py-2.5 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-neutral-300 text-xs font-semibold transition-colors">Vazgeç</button>
