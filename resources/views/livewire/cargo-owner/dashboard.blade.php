...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <a href="{{ route('cargo-owner.loads.index') }}" wire:navigate class="bg-neutral-900 border border-neutral-800/80 rounded-2xl p-5 hover:border-neutral-700 transition-all duration-200 group">
            <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-semibold text-neutral-400">Yayındaki ilanlar</span>
        ...
        <a href="{{ route('cargo-owner.loads.index') }}" wire:navigate class="bg-neutral-900 border border-neutral-800/80 rounded-2xl p-5 hover:border-neutral-700 transition-all duration-200 group">
            <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-semibold text-neutral-400">Bekleyen teklifler</span>
            ...
            <div class="text-3xl font-extrabold text-white tracking-tight">{{ $pendingOffersCount }}</div>
            <div class="text-xs text-neutral-500 mt-2">Değerlendirmenizi bekliyor</div>
        </a>

        <a href="{{ route('cargo-owner.shipments.index') }}" wire:navigate class="bg-neutral-900 border border-neutral-800/80 rounded-2xl p-5 hover:border-neutral-700 transition-all duration-200 group">
            <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-semibold text-neutral-400">Yoldaki sevkiyatlar</span>
            ...
            <div class="text-3xl font-extrabold text-white tracking-tight">{{ $inTransitCount }}</div>
            <div class="text-xs text-neutral-500 mt-2">Şoför yükü taşıyor</div>
        </a>

        <a href="{{ route('cargo-owner.finance.index') }}" wire:navigate class="bg-neutral-900 border border-neutral-800/80 rounded-2xl p-5 hover:border-neutral-700 transition-all duration-200 group">
            <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-semibold text-neutral-400">Güvenli havuzda</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden flex flex-col">
            <div class="p-5 border-b border-neutral-800 flex items-center justify-between bg-neutral-900/60">
                <h3 class="text-sm font-bold text-white tracking-tight">Son ilanlarınız</h3>
            ...
            @if($latestLoads->isEmpty())
                <div class="p-10 text-center space-y-3">
                    <p class="text-sm font-bold text-white">Henüz ilanınız yok</p>
                    <p class="text-xs text-neutral-400 max-w-sm mx-auto">İlk ilanınızı yayınladığınızda şoförlerden gelen teklifler burada görünür.</p>
                        ...
                        <div class="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                            <div class="space-y-1 min-w-0">
                                <div class="flex flex-wrap items-center gap-2">
                                    <span class="px-2 py-0.5 rounded-md bg-neutral-800 text-neutral-300 font-mono text-[11px] font-bold">#{{ $load->id }}</span>
                                    <span class="px-2 py-0.5 rounded-full text-[10px] font-bold border
                                        {{ $load->status === 'active_seeking' ? 'bg-blue-500/10 border-blue-500/20 text-blue-400' : ($load->status === 'completed' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : ($load->status === 'cancelled' ? 'bg-neutral-800 border-neutral-700 text-neutral-400' : ($load->status === 'disputed' ? 'bg-rose-500/10 border-rose-500/20 text-rose-400' : 'bg-brand-500/10 border-brand-500/20 text-brand-400'))) }}">
                                        {{ $load->statusLabel() }}
                                    </span>
                                    <span class="text-neutral-500">{{ $load->created_at?->format('d.m.Y H:i') }}</span>
                                </div>
                                <div class="text-sm font-semibold text-white truncate">{{ $load->pickup_location }} <span class="text-brand-500">&rarr;</span> {{ $load->delivery_location }}</div>
                            ...
                            <div class="flex items-center justify-between sm:justify-end gap-3 shrink-0">
                                <span class="font-mono font-bold text-white">{{ number_format((float) ($load->price ?? 0), 2, ',', '.') }} ₺</span>
                                @if($load->status === 'active_seeking')
                                    <a href="{{ route('cargo-owner.loads.offers', $load->id) }}" wire:navigate class="px-3 py-1.5 rounded-lg bg-neutral-800 hover:bg-neutral-700 text-white font-semibold">Teklifler</a>
                                @elseif(in_array($load->status, ['driver_assigned', 'on_the_way', 'delivered', 'disputed', 'completed'], true))
                                    <a href="{{ route('cargo-owner.shipments.show', $load->id) }}" wire:navigate class="px-3 py-1.5 rounded-lg bg-neutral-800 hover:bg-neutral-700 text-white font-semibold">Sevkiyat</a>
                                @endif
                            </div>
                        </div>
                    @endforeach
        ...
        <div class="space-y-6">
            <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-5 space-y-4">
                <h3 class="text-sm font-bold text-white tracking-tight">Son aktif sevkiyat</h3>
                ...
                @if($activeShipment)
                    ...
                    <div class="space-y-3 text-xs">
                        <div class="flex flex-wrap items-center gap-2">
                            <span class="px-2 py-0.5 rounded-full bg-brand-500/10 border border-brand-500/20 text-brand-400 text-[10px] font-bold">{{ $activeShipment->statusLabel() }}</span>
                            <span class="px-2 py-0.5 rounded-full bg-neutral-800 border border-neutral-700 text-neutral-300 text-[10px] font-bold">{{ $activeShipment->escrowLabel() }}</span>
                        </div>
                        <div class="text-sm font-semibold text-white">{{ $activeShipment->pickup_location }} <span class="text-brand-500">&rarr;</span> {{ $activeShipment->delivery_location }}</div>
                        <div class="p-3 rounded-xl bg-neutral-950 border border-neutral-800 space-y-1.5">
                            <div class="flex items-center justify-between gap-3">
                                <span class="text-neutral-500">Şoför</span>
                                <span class="text-neutral-200 font-semibold text-right">{{ $driverUser?->full_name ?: 'Henüz atanmadı' }}</span>
                            </div>
                            <div class="flex items-center justify-between gap-3">
                                <span class="text-neutral-500">Araç</span>
                                <span class="text-neutral-200 text-right">{{ $vehicle ? trim(($vehicle->brand ?? '').' '.($vehicle->model ?? '')) ?: (\App\Models\DriverVehicle::getVehicleTypes()[$vehicle->vehicle_type] ?? $vehicle->vehicle_type) : 'Araç bilgisi yok' }}</span>
                            </div>
                            <div class="flex items-center justify-between gap-3">
                            ...
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <span class="text-neutral-200">{{ $activeShipment->pickup_date?->format('d.m.Y') ?? '—' }}</span>
                            </div>
                        </div>
                ...
                @else
                    <p class="text-xs text-neutral-400 leading-relaxed">Henüz aktif bir sevkiyatınız yok. Bir teklifi kabul edip ödemeyi tamamladığınızda sevkiyat burada görünür.</p>
                @endif
            </div>

            <div class="p-3 rounded-xl bg-neutral-900 border border-neutral-800 flex items-center justify-between">
