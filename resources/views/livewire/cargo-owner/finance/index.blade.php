...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-5 sm:col-span-2 lg:col-span-1">
            <span class="text-xs font-semibold text-neutral-500 dark:text-neutral-400 block mb-3">Aylık harcama (son 6 ay)</span>
            <div class="flex items-end gap-2 h-20">
                @foreach($monthly as $month => $amount)
                    ...
                    <div class="flex-1 flex flex-col items-center gap-1 min-w-0" title="{{ $monthNames[(int) $mon] }} {{ $year }}: {{ number_format($amount, 2, ',', '.') }} ₺">
                        <div class="w-full rounded-t-md {{ $amount > 0 ? 'bg-brand-500' : 'bg-neutral-100 dark:bg-neutral-800' }}" style="height: {{ max(4, (int) round($amount / $monthlyMax * 56)) }}px"></div>
                        <span class="text-[11px] text-neutral-500">{{ $monthNames[(int) $mon] }}</span>
                    </div>
                @endforeach
            </div>
        </div>
    ...
    <div class="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl overflow-hidden">
        <div class="p-5 border-b border-neutral-200 dark:border-neutral-800 flex items-center justify-between">
            <h3 class="text-sm font-bold text-neutral-900 dark:text-white tracking-tight">Ödeme geçmişi</h3>
            <span class="text-xs text-neutral-500 dark:text-neutral-400">{{ $orders->total() }} kayıt</span>
        </div>
        ...
        @if($orders->hasPages())
            <div class="p-4 border-t border-neutral-200 dark:border-neutral-800 text-xs">{{ $orders->links() }}</div>
        @endif
    </div>

    <div class="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl overflow-hidden">
        <div class="p-5 border-b border-neutral-200 dark:border-neutral-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <h3 class="text-sm font-bold text-neutral-900 dark:text-white tracking-tight">Faturalar</h3>
            @if($invoices->isNotEmpty())
                <button type="button" onclick="window.print()" class="px-3 py-1.5 rounded-lg bg-neutral-100 dark:bg-neutral-800 hover:bg-neutral-200 dark:hover:bg-neutral-700 text-neutral-900 dark:text-white font-medium text-[11px] transition-colors inline-flex items-center gap-1.5">
